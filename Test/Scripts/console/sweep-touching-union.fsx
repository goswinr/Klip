    ...
    if res.Count = 0 then
        ...
        if r0.PointCount = 8 && areaOkK res pathK then
            ...
            if !thisScaleFailCount < 3 then
                Printfn.purple $"  ERROR: BAD AREA: {r0.PointCount} points at scale {scale}, rotation {rot} moveX {moveX}, moveY {moveY}"
        else
            incr failCount
            incr thisScaleFailCount
            if !thisScaleFailCount < 3 then
...
let run() =
    ...
    Printfn.blue $"Total Success count: {successCount.Value}"
    Printfn.red $"Total ERROR count: {failCount.Value}"
